...
class BouncingBall:
    ...
    def step(self):
        ...
        speedX = self.dirx * self.speed
        speedY = self.diry * self.speed
        dx = self.displacement(self.center[0], speedX, 800)
        dy = self.displacement(self.center[1], speedY, 600)
        if self.colDetect(self.center[0], speedX, 800):
            self.dirx = -self.dirx
        if self.colDetect(self.center[1], speedY, 600):
            self.diry = -self.diry
...
if __name__ == '__main__':
    ...
    for i in range(5):
        bouncing_balls.append(random_ball(canvas, 10, 60, 5.0, 15.0, 800, 600))
    bouncing_balls.append(BouncingBall(canvas, (0, 600), 20, random_color, 0, 0))
