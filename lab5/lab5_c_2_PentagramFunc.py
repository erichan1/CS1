...
def drawPentagramThing(x, y, radius, points):
    '''draws a pentagram thing (idk what to call it)
    with coordinates at x,y, and with some radius and some number of points'''
    ...
    for i in range(points):
        for j in range(i+1, points):
            line = canvas.create_line(pointList[i][0], pointList[i][1], pointList[j][0], pointList[j][1], fill = color)
